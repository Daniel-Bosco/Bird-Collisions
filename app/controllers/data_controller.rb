class DataController < ApplicationController
  ...
  def index
    @data = Datum.all
    @data = @data.where(id: params[:datum_id]) unless params[:datum_id].blank?
    @data = @data.page(params[:page] || 1)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @datum.save
        format.html { redirect_to new_collision_path, notice: "Datum was successfully created." }
  ...
  private
    ...
    def datum_params
      params.require(:datum).permit(:date, :flight_phase, :airport_id, :flight_id, :weather_id, :report_id)
    end
end
