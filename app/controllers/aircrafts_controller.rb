class AircraftsController < ApplicationController
  ...
  def index
    ...
    @aircrafts = @aircrafts.where("model LIKE :filter", filter: "%" + params[:filter] + "%") unless params[:filter].blank?
    @aircrafts = @aircrafts.order(params[:by] + " " + params[:order]) if params[:sort] == "true"
    @aircrafts = @aircrafts.page(params[:page] || 1)
  end
  ...
  private
    ...
    def aircraft_params
      params.require(:aircraft).permit(:category, :certification, :model)
    end
end
