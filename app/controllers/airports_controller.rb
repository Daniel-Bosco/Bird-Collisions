class AirportsController < ApplicationController
  ...
  def index
    ...
    @airports = @airports.where("name LIKE :filter", filter: "%" + params[:filter] + "%") unless params[:filter].blank?
    @airports = @airports.order(params[:by] + " " + params[:order]) if params[:sort] == "true"
    @airports = @airports.page(params[:page] || 1)
  end
  ...
  private
    ...
    def airport_params
      params.require(:airport).permit(:name, :category, :city_id)
    end
end
