class BirdsController < ApplicationController
  ...
  def index
    @birds = Bird.all
    @birds = @birds.where("common_name LIKE :filter OR scientific_name LIKE :filter", filter: "%" + params[:filter] + "%") unless params[:filter].blank?
    @birds = @birds.joins(:collisions).order(params[:by] + " " + params[:order]).distinct if params[:sort] == "true"
    @birds = @birds.page(params[:page] || 1)
  end
